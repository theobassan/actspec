// Composite executor — evolved from api-ergonomics spike to accept matrixCtx.
...
import type { ParsedAction, ParsedStep, RunInput, RunResult, StepResult, Annotation } from './types.js';
...
export async function runComposite(opts: CompositeRunOptions): Promise<RunResult> {
  ...
  try {
    for (const step of opts.action.runs.steps ?? []) {
      ...
      const ctx = buildContexts(opts.input, inputValues, stepsCtx, accumulatedEnv, jobStatus, opts.needsCtx, opts.matrixCtx);
      ...
      if (!shouldRun) {
        const skipped: StepResultInternal = {
          ...
          outputs: {}, if: ifRecord, stdout: '', stderr: '', _envWrites: {},
        };
        steps.push(skipped);
      ...
      if (step.run !== undefined) {
        result = await execShellStep({ stepId, stepName, step, input: opts.input, inputValues, accumulatedEnv, stepEnvRaw, stepsCtx, jobStatus, workspace, needsCtx: opts.needsCtx, matrixCtx: opts.matrixCtx });
      } else if (step.uses !== undefined) {
        result = await execUsesStep({ stepId, stepName, step, input: opts.input, inputValues, accumulatedEnv, stepEnvRaw, stepsCtx, jobStatus, mocks: opts.mocks, actionDir: opts.actionDir, needsCtx: opts.needsCtx, matrixCtx: opts.matrixCtx });
      } else {
    ...
    const finalCtx = buildContexts(opts.input, inputValues, stepsCtx, accumulatedEnv, jobStatus, opts.needsCtx, opts.matrixCtx);
    ...
    return makeRunResult({
      ...
      outputs, steps, env: accumulatedEnv, annotations: allAnnotations,
...
interface StepCtx {
  ...
  workspace: string; needsCtx?: Record<string, unknown>; matrixCtx?: Record<string, unknown>;
}

async function execShellStep(opts: StepCtx): Promise<StepResultInternal> {
  const ctx = buildContexts(opts.input, opts.inputValues, opts.stepsCtx, opts.accumulatedEnv, opts.jobStatus, opts.needsCtx, opts.matrixCtx);
...
interface UsesStepCtx {
  ...
  accumulatedEnv: Record<string, string>; stepEnvRaw: Record<string, string>;
  stepsCtx: Record<string, unknown>; jobStatus: JobStatus;
  mocks: MockRegistry; actionDir: string; needsCtx?: Record<string, unknown>; matrixCtx?: Record<string, unknown>;
}
...
async function execUsesStep(opts: UsesStepCtx): Promise<StepResultInternal> {
  ...
  const currentEnv = buildEnvVars(opts.input, opts.inputValues, opts.accumulatedEnv, opts.stepEnvRaw, opts.matrixCtx);
  ...
  if (ref.startsWith('./') || ref.startsWith('../')) {
    ...
    return {
      ...
      stdout: childResult.stdout, stderr: childResult.stderr, _envWrites: childResult.env,
    };
  }
  ...
  return {
    ...
    outputs: {}, uses: { ref, mocked: false }, stdout: '', stderr: '', _envWrites: {},
  };
}
...
function resolveIf(
  ...
  if (ifExpr === undefined) return { shouldRun: jobStatus.success };
